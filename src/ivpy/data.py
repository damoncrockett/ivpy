...
def _colfilter(pathcol=None,featcol=None,sample=None,ascending=False,ycol=None):

    global ATTACHED_DATAFRAME
    global ATTACHED_PATHCOL    

    """
    Managing user supplied data. If a DataFrame is attached, user can pass an empty montage
    ...
    if featcol is not None:        
        if isinstance(featcol, pd.Series):
            if featcol.index!=pathcol.index: # too strong a criterion?
                raise ValueError("Image paths and image features must have same indices")
    ...
    if ycol is not None:        
        if isinstance(ycol, pd.Series):
            if ycol.index!=pathcol.index: # too strong a criterion?
                raise ValueError("Image paths and image features must have same indices")
        elif isinstance(ycol, basestring):
            if ATTACHED_DATAFRAME is None:
                raise ValueError("""No DataFrame attached. Variable 'ycol' must be a pandas
                    Series""")
            else:
                tmp = copy.deepcopy(ATTACHED_DATAFRAME)
                ycol = tmp[ycol]
            ...
            raise ValueError("If supplied, 'ycol' must be a string or a pandas Series")
    
    """
    If user supplies a number for 'sample', we sample pathcol and subset feat/ycol to these 
    indices. We do this because later, we might sort by feat/ycol. If so, we will want to then
    subset pathcol by feat/ycol, and this won't work if pathcol is missing a bunch of the indices
    ...
    """
    If user supplies featcol, we sort featcol and apply to path/ycol. At this point, if 
    featcol is not None, we know it is a pandas Series (either user-supplied or from the 
    attached df). We also know it is the same length as pathcol - even if pathcol got 
    sampled above. Note that for scatterplots, sorting makes no difference. But it's easier
    to reuse this function for all plots. Note that your working DataFrame is unchanged.
    """
    if featcol is not None:
